...
To interact with the workspace GraphRAG knowledge graph, use the `workflow.graphrag.*` REPL command namespace via your agent's MCP Server REPL wrapper, which invokes `mcpserver-repl --agent-stdio`. When sending requests to the REPL over stdio directly, each message must be one single-line JSON request envelope per message, not formatted YAML. GraphRAG combines graph-based retrieval with semantic search, enabling richer context retrieval than vector-only approaches.
