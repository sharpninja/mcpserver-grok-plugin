...
function resolveAgentName() {
  return process.env.MCP_AGENT_NAME || process.env.PLUGIN_AGENT_NAME || process.env.PLUGIN_AGENT_DEFAULT || process.env.MCP_PLUGIN_HOST || 'default';
}
...
function spawnRepl() {
  ...
  const replArgs = ['--agent-stdio', '--agent', agent];
  if (REPL_BIN.endsWith(".js")) {
    return spawn(process.execPath, [REPL_BIN, ...replArgs], {
      stdio: ["pipe", "pipe", "ignore"],
      env: process.env,
    });
  }
  return spawn(REPL_BIN, replArgs, {
    stdio: ["pipe", "pipe", "ignore"],
    env: process.env,
  });
}
