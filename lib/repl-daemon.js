...
function spawnRepl() {
  ...
  const agent = process.env.MCP_AGENT_NAME || process.env.PLUGIN_AGENT_NAME || process.env.PLUGIN_AGENT_DEFAULT || 'default';
  const replArgs = ['--agent-stdio'];
  if (agent && agent !== 'default') {
    replArgs.push('--agent', agent);
  }
